...
def features(data_dir, file_name, percent_to_read):
    ...
    processed_time, processed_std = GetAvgTime(dl.csv_data_dict["combined_eventtimestamp"]).run()
    login1, login2, login3, login4  = LoginTime(dl.csv_data_dict["sessionstarttime_hour"],\
        dl.csv_data_dict["sessionstarttime_minute"],dl.csv_data_dict["sessionstarttime_weekday"]).run()
    time = ProcessTime(dl.csv_data_dict["combined_pagelocation"],dl.csv_data_dict["combined_eventtimestamp"]).run()
