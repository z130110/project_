...
def t2(loaded_data):
    save_dir = "/data/numpy_conversions/finalrepv3/"
    avg_time, avg_std = GetAvgTime(loaded_data.csv_data_dict["all_eventtimestamp"]).run()
    #avg_time = avg_time.tolist()
    #avg_std = avg_std.tolist()
    
    res_np = loaded_data.to_numpy([
        avg_time, \
        avg_std,\
        loaded_data.csv_data_dict["target"]])
    #train, test = split_and_balance_data(res_np)
    filename = save_dir + "T2AVGTEST_"
    #np.save(filename + "train.npy", train)
    np.save(filename + "all.npy", res_np)
...
if __name__ == "__main__":
    save_csv_to_np("data11", "1.0_10_3.csv",0.5)
    #save_csv_to_np("data11", "test.csv")
